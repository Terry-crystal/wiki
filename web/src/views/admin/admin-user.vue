        ...
        >
            ...
            <p>
                <a-form layout="inline" :model="param">
                    <a-form-item>
                        <a-input v-model:value="param.loginName" placeholder="输入要查询的账户"></a-input>
                    </a-form-item>
                ...
                <template v-slot:action="{ text, record }">
                    <a-space size="small">
                        ...
                        <a-popconfirm
                                ...
                                ok-text="是"
                                cancel-text="否"
                                @confirm="handleUserDelete(record.id)"
                        >
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const handleQuery = (params: any) => {
                loading.value = true;
                users.value = [];    //清空现有数据，就不会出现编辑之后还是原来数据的清空
                axios.get("/user/list", {
                    params: {
                        page: params.page,
                        size: params.size,
            ...
            const add = () => {
                modalVisible.value = true;  //显示模糊框
                user.value = {};   //将模糊框内部数据清空
            };

            /**
             * 删除
             */
            const handleUserDelete = (id: string) => {   //Long类型对应前端类型为number类型
